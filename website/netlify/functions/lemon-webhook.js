...
// Variant ID-jevi / Uslugama iz Lemon Squeezy dashboard-a
...
exports.handler = async (event) => {
  ...
  const supabaseUrl = (process.env.SUPABASE_URL || 'https://rcukparptzzyssqdmydt.supabase.co').replace(/\/+$/, '');
  ...
  if (eventName === 'subscription_cancelled' || eventName === 'subscription_expired') {
    ...
    variantIdStr === PRO_VARIANT_ID || 
    productName.includes('pro') || 
    variantName.includes('pro')
  ) {
    newPlan = 'pro';
  } else if (
    variantIdStr === ELITE_VARIANT_ID || 
  ...
  try {
    const updateRes = await fetch(`${supabaseUrl}/rest/v1/user_profiles?id=eq.${encodeURIComponent(userId)}`, {
      method: 'PATCH',
      headers: sbHeaders,
